...
const GetTaquitos = ()=>{
    ...
    const tacosArray = data.map(element =>{
        console.log(element);
        const{id, name, quantity, pica} = element;
        return{tacoId: id, tacoName: name, tacoQuantity: quantity, tacoPica: pica};
    });

    await tacosArray.forEach(element => {
        //console.log(element);

        tacosList.innerHTML += 
        `<li>
             ${element.tacoId}-Name: ${element.tacoName}
             <ul>
                    <div>-Location: ${element.tacoQuantity}</div>
                    <div>-Dimension: ${element.tacoPica}</div>
             </ul>
        </li>`
     });*/
